...
每個 `--color-*` 只有一個值；`color-scheme` 是 `dark`，宣告在
[`app/themes/default.css`](../apps/web/app/themes/default.css) 裡。

`color-scheme` 不能省 —— 原生表單控制項與捲軸只認它，沒有任何 token 管得到它們。
少了它，深色底上的 `<select>` 與捲軸會維持亮色，那是最容易漏掉的一半。
它**宣告在主題檔而不是語意層**，理由跟 `--color-*` 一樣：「這個 app 是深色還是亮色」
是那一份主題的事。寫在語意層等於把它擺在介面外面 —— 換一份亮色主題時顏色全換了、
`<select>` 與捲軸卻沒有，而規則 r 只比對 `--color-*`，那件事不會有任何紅燈（規則 w 在守）。
...
控制項邊界與 focus 外環要 3:1，狀態色的文字對它自己的底也問一次。**`app/themes/` 底下每多一份主題就多跑一輪同一組門檻**，
...
介面裡有三樣東西**不是**單純的顏色對應，值得先知道它們為什麼在這裡：

- **`color-scheme`**（規則 w）—— 原生控制項只認它，沒有 token 管得到。
- **狀態色 `--color-{success,warning,danger,info}-{fg,bg}`** —— 色相跨主題共用
  （`--ds-*-600` 那一階不隨 `data-theme` 變），但**明暗要拿這份主題的底色推導**：
  fg 往白提亮、bg 混頁面底色，兩者都只在深色底上成立。所以推導留在主題層，
  來源留在自己這一側（見〈跨主題共用的東西不要接到單一 DS〉）。
- **`--color-shadow`** —— 陰影的顏色。幾何與濃度在原始層（`--ds-shadow-*` 與
  `--ds-shadow-alpha-*`），語意層把兩者接起來。「陰影是黑的」是亮色 UI 的假設，
  鎖在原始層的話主題換不掉它（規則 x 在守）。
...
| x | `--ds-shadow-*` 不可自帶色值 —— 陰影顏色由主題的 `--color-shadow` 決定 |
...
| 4 | 複製 `app/themes/default.css` 成 `app/themes/<ds>.css`，選擇器改 `:root[data-theme="<ds>"]`，只換右手邊（`color-scheme` 也要跟著換 —— 上游是亮色色票的話那一行是 `light`） | 少宣告哪個顏色會被逐行列出來；漏了 `color-scheme` 規則 w 會指名那一份 |
...
- **語意名字要從 vendor 檔拿掉。** DS 產出通常把原始值與語意混在同一份
  ...
  `--surface-*`、`--text-*`、`--border-*`，以及任何 `--color-*` 開頭的名字（狀態色現在也在這一組）。
...
上游的色票是照著它自己的畫面挑的，**通常只有一組亮色**。這時**先問一句：這個產品要是
亮色的嗎？** 兩條路的成本差很多：

- **要 —— 那就直接導入成一份亮色主題**（`color-scheme: light`），內建的 `default` 深色
  留著當第二份。沒有推導成本，這是預設路徑。
- **要深色（或兩份都要留著能切）—— 才需要把缺的那一半推導出來**：表面要幾階、
  文字要幾階、品牌色在深色底上要不要提亮。（量級大約是：表面補 5 階、文字補 4 階、
  品牌補 3 階。）

在問這句之前值得先確認一件事：**「只有亮色」常常是匯出的問題，不是設計的問題。**
Figma Variables 的一組 collection 可以帶多個 mode，匯出時只選了一個 mode 很常見 ——
回頭要第二個 mode，比自己推導便宜得多。
...
狀態色（success／warning／danger／info）的**色相**在這個 repo 跨主題共用（成功就是綠的），
`primitives.css` 的 `--ds-*-600` 那一階 `default` 主題也在用。把它接到某一套 DS 的 vendor 上，
等於讓內建主題也依賴那套 DS —— 那套一旦被刪掉或換掉，內建主題跟著壞。
**值可以照抄，來源要留在自己這一側。**

**共用的只有色相，不包含明暗。** `--color-*-fg`／`--color-*-bg` 是拿那一階和這份主題的
底色混出來的，所以它們跟其他 `--color-*` 一樣，每份主題各給一次（規則 r 涵蓋它們）。
陰影同理：`--ds-shadow-*` 是幾何、`--color-shadow` 是顏色。
