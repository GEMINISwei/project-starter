...
與捲軸會維持亮色，那是最容易漏掉的一半。它為什麼算在主題介面裡，見下一節。
...
- **`color-scheme`**（規則 w）—— 原生表單控制項與捲軸只認它，沒有任何 token 管得到它們。
  擺在語意層就是擺在介面外面：換一份亮色主題時 `--color-*` 全換了、`<select>` 與捲軸卻沒有，
  而規則 r 只比對 `--color-*`，那件事**不會有任何紅燈**。這條是它從語意層搬過來的理由。
...
它宣告的那組 `--color-*` 與 `color-scheme` 就是契約本身。這裡不抄一份名單 —— 抄了就會漂。
