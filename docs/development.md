...
刻意訂得寬（見〈測試與覆蓋率〉），刪掉幾支小測試照樣綠燈，但那個名字一旦查無此人就是紅的。
...
相依由 dependabot 每週開 PR，多數直接合。跨 major 或一次升一批時，照這三條：
...
單支測試的 timeout 會**跟著放慢一起放大**。上面那 90 秒是照 `next dev` 的編譯時間訂的，
沒有含放慢那一筆 —— 不放大的症狀是「headed 跑到一半 timeout，換回 headless 就過」。
...
3. **這些是防退步的地板，不是重構目標**（同覆蓋率門檻，理由見上面〈測試與覆蓋率〉）。
