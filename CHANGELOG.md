...
- **修掉五處懸空的指路 —— 全是 `docs/ci-cd.md` 那次拆分自己留下的。** 把〈分支保護〉
  ...
  **漏掉程式碼註解裡指向那兩節的**：`scripts/check-ci.sh` 三處、
  `.github/workflows/ci.yml` 兩處，都還寫著 `docs/development.md 的〈分支保護〉`。
  `check-docs` 的 `DOCS` 只含 `.md`，所以這五處指向一個已經不存在的小節而**永遠不會紅**。
  順帶修掉同一行的另一個過期說法：`check-ci.sh` 沒有 ruleset 檔時印的
  「它是選配的」與文件現在的立場（「ruleset 不是選配的」）相反 ——
  那句想講的其實是「下游可以整套不用 `.github/`」，照著改。
  這一輪是反方向掃出來的（程式碼註解 → 文件），另外十條指向 `development.md` 的
  逐條確認都還成立。
