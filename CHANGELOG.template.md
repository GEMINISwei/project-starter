...
**這份是模板自己的，記錄這個 repo 作為基底時改了什麼。**

從這個模板開出來的新專案：**把這份檔案刪掉**，然後在
[`CHANGELOG.md`](CHANGELOG.md) 寫你自己的。兩份分開就是為了讓那個動作只是「刪一個檔案」，
不必從別人的紀錄裡挑出哪幾行是自己的。步驟見 [`TEMPLATE.md`](TEMPLATE.md)。

條目只寫**改了什麼**，版本對應 `apps/api/app/config.py` 的 `APP_VERSION`。
怎麼寫與日常條目要累積在哪裡，見
...
- 變更紀錄分成兩份：模板自己的在 `CHANGELOG.template.md`，根目錄的
  [`CHANGELOG.md`](CHANGELOG.md) 留給用這個模板開出來的專案。開案時刪掉前者即可，
  不必從別人的紀錄裡挑出哪幾行是自己的。
